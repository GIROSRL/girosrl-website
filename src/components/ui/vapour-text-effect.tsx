...
/**
 * VaporizeTextCycle — effetto "testo che si vaporizza" parola per parola.
 * Fonte: prompt utente (pattern 21st.dev). Adattato a React 19 + TS strict.
 *
 * Usi consigliati in GIRO:
 * - Hero pagina `/servizi/intelligenza` (AI che "pensa": testo che si ricompone)
 * - Verbo imperativo hero `/percorsi/parti` — "PARTI / CRESCI / TRASFORMA"
 * - NON usare nell'home principale (conflitto con scena 3D pesante)
 */
...
export default function VaporizeTextCycle({
  ...
  const isInView = useIsInView(wrapperRef as RefObject<HTMLElement>)
      ...
      switch (animationState) {
        ...
        case "fadingIn": {
          ...
          particlesRef.current.forEach((particle) => {
            ...
            const col = particle.color.replace(/[\d.]+\)$/, `${opacity})`)
            ctx.fillStyle = col
...
const SeoElement = memo(({ tag, texts }: { tag: TagValue; texts: string[] }) => {
  const style = useMemo(
    () => ({
...
function handleFontChange({
  ...
  currentFont: string
  lastFontRef: React.RefObject<string | null>
...
function transformValue(
  ...
  const [inputMin, inputMax] = inputRange as [number, number]
